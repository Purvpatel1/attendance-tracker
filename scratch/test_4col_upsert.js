...
async function test4ColUpsert() {
  const email = `upsert_4col_${Date.now()}@example.com`;
  const password = 'Password123!';

  const { data: sData } = await supabase.auth.signUp({
    email,
    password,
    options: { data: { full_name: '4Col Test', branch: 'Computer Engineering (CE)', batch: 'CE1' } }
  });
  ...
  if (user) {
    ...
    const subjectUuid = stringToUuid(rawSubjectCode);
    ...
    console.log('Testing 4-column upsert payload:', payload);
    const { data: insData, error: insErr } = await supabase
      .from('attendance_logs')
      .upsert(payload, { onConflict: 'student_id,date,slot_id,subject_id' })
      .select();

    console.log('Upsert 4-Col Result:', insData);
    console.log('Upsert 4-Col Error:', insErr);
  }
}
